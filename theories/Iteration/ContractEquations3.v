...
Lemma test : forall c0 c1, co_eq c0 c1 -> co_eq c1 c0.
Proof.
pcofix CIH. intros. pfold. punfold H.
- induction H.
2: { constructor. auto. auto. intros. left. apply CIH. Guarded.
  *
...
Lemma co_eq_soundness : forall (c0 c1 : Contract), co_eq c0 c1 -> Bisimilarity c0 c1.
Proof.
cofix CIH.
intros c0 c1 H9. punfold H9. induction H9.
- rewrite <- matches_eq_iff_bisimilarity. 
  split;intros;c_inversion.
- pfold. constructor.
  * intros. left. apply CIH. Guarded.
induction H9.
25 : {
  * 
    constructor.
    ** intros. apply CIH. Guarded.

induction H9;intros; try solve [split;intros;c_inversion].
...
Lemma c_eq_soundness : forall (c0 c1 : Contract), c_eq bot2 c0 c1 -> (forall s : Trace, s =~ c0 <-> s =~ c1).
Proof.
intros c0 c1 H. induction H ;intros; try solve [split;intros;c_inversion].
  ...
  * split;intros; c_inversion; econstructor;eauto;
                                ...
                                | rewrite IHc_eq1
                                | rewrite IHc_eq2];auto.
  * specialize H2 with 0. unfold bot2 in H2. contradiction.
Qed.
...
Hint Resolve c_eq_gen_mon : paco.
(*
Theorem my_principle : forall c0 c1 r, c_eq bot2 c0 c1 \/ paco2 c_eq r c0 c1 -> (P c0 c1.
Proof.
intros. destruct H.

Section principle.
  Variable P : Contract -> Contract -> Prop.
  Hypothesis H0 : forall c0 c1, c_eq bot2 c0 c1 -> P c0 c1.

  Hypothesis H1 : forall l0 l1, length l0 = length alphabet -> length l0 = length alphabet ->
                                (forall n, co_eq (nth n l0 Failure) (nth n l1 Failure) : Prop) ->
                                P (Σe alphabet l0) (Σe alphabet l1).


  Theorem my_principle : forall c0 c1, c_eq r c0 c1 -> P c0 c1.
Proof.
intros. punfold H.

Lemma principle : forall c0 c1 P, (c_eq bot2 c0 c1 -> P c0 c1) ->
                                  (forall l0 l1, length l0 = length alphabet ->
                                                 length l1 = length alphabet    bot2) ->  -> (co_eq c0 c1 -> P c0 c1).
...
Lemma Σe_not_empty : forall l0 l1, [] =~ Σe l0 l1 -> False.
Proof.
induction l0.
- unfold Σe. simpl. intros. inversion H.
- destruct l1.
  * unfold Σe.  simpl.  intros.  inversion H. 
  * rewrite Σe_cons. intros. inversion H.
    ** inversion H2. apply app_eq_nil in H4. destruct H4.
       subst. inversion H7.
    ** eauto. 
Qed.
Check nth.
Lemma Σe_match_cons : forall l0 l1 e, (forall n, (forall s, s =~ nth n l0 Failure <-> s =~ nth n l1 Failure ))  ->
                  (forall s, s =~ e \ Σe alphabet l0 <-> s =~ e \ Σe alphabet l1).
Proof. Admitted.

Lemma final_test : forall l0 l1 e, (forall n : nat, paco2 c_eq bot2 (nth n l0 Failure) (nth n l1 Failure)) ->
                    co_eq (e \ Σe alphabet l0) (e \ Σe alphabet l1).
Proof. Admitted.
...
Proof.
cofix CIH.
intros c0 c1 H9. (*rewrite <- matches_eq_iff_bisimilarity'.*) punfold H9. 
induction H9.
25 : {
  * 
    constructor.
...
induction H9;intros; try solve [split;intros;c_inversion].
  ...
  * now symmetry.
  * eauto using iff_trans.
  * split;intros; inversion H; [ rewrite IHc_eq1 in * 
                                | rewrite IHc_eq2 in *
                                | rewrite <- IHc_eq1 in * 
                                | rewrite <- IHc_eq2 in *];auto with cDB.
  * split;intros; c_inversion; constructor;
                                [ rewrite <- IHc_eq1
                                | rewrite <- IHc_eq2
                                | rewrite IHc_eq1
  ...
  * split;intros; c_inversion; econstructor;eauto;
  ...
  destruct s.
    ** split;intros; apply Σe_not_empty in H; contradiction.
    ** repeat rewrite derive_spec_comp. generalize dependent s.
       apply Σe_match_cons. intro n. rewrite matches_eq_iff_bisimilarity.
       specialize H2 with n. destruct H2. auto.
       unfold bot2 in H. contradiction.
Qed.

punfold H.  induction H.
- rewrite <- matches_eq_iff_bisimilarity. apply c_eq_soundness.
- constructor. apply CIH. apply H. Guarded. punfold H. simpl in H. pfold.  constructor.
- intros. .

Add Parametric Relation : Contract c_eq_base
...
signature c_eq_base ==> c_eq_base ==> c_eq_base as c_eq_base_par_morphism.
Proof.
  intros. auto with eqDB.
Qed.

Add Parametric Morphism : CPlus with
signature c_eq_base ==> c_eq_base ==> c_eq_base as c_eq_base_plus_morphism.
Proof.
  intros. auto with eqDB.
Qed.

Add Parametric Morphism : CSeq with
signature c_eq_base ==> c_eq_base ==> c_eq_base as c_eq_base_seq_morphism.
Proof.
  intros. auto with eqDB.
Qed.

Lemma co_eq_soundness : forall c0 c1, c_eq bot2 c0 c1 -> (forall s, s =~ c0 <-> s=~c1).
Proof.




Lemma test: forall c0 c1, c_eq (c_eq bot2) c0 c1 -> (forall s, s=~ c0 <-> s =~ c1).
Proof. Admitted.


Lemma testupaco : forall l0 l1, (forall n : nat, upaco2 c_eq bot2 (nth n l0 Failure) (nth n l1 Failure)) <->
                           (forall n : nat, paco2 c_eq bot2 (nth n l0 Failure) (nth n l1 Failure)).
Proof.
intros. split.
- intros. specialize H with n. pclearbot.  auto.
- intros. left. auto.
Qed.

Lemma co_eq_soundness : forall c0 c1, co_eq c0 c1 -> Bisimilarity c0 c1.
Proof.
pcofix CIH.
intros. punfold H0. induction H0.
25: { pfold. constructor.
- intros. setoid_rewrite test in H2. generalize dependent s. rewrite matches_eq_iff_bisimilarity.
     generalize dependent l1. generalize dependent l0. pcofix CIH.
    intros. pfold.  constructor. 2: { repeat rewrite Σe_not_nu. auto. }
    intros. right.
 auto using Σe_not_nu.
- intros.
pcofix CIH. intros.
...
Lemma co_eq_soundness : forall c0 c1, co_eq c0 c1 -> (forall s, s=~c0 <-> s=~c1).
Proof.
intros. punfold H. induction H.
25: { setoid_rewrite test in H2. generalize dependent s. rewrite matches_eq_iff_bisimilarity.
     generalize dependent l1. generalize dependent l0. pcofix CIH.
    intros. pfold.  constructor. 2: { repeat rewrite Σe_not_nu. auto. }
    intros. right.
 auto using Σe_not_nu.
...
Lemma test3: forall c0 c1, c_eq (upaco2 c_eq bot2) c0 c1 -> 
c_eq (c_eq bot2) c0 c1.
Proof.
intros. induction H.
25: {   apply c_co_sum;  auto. setoid_rewrite testupaco in H2.
intros. specialize H2 with n. punfold H2. destruct H2. pclearbot. punfold H2. auto. iwht n. constructor. 

Lemma test2: forall c0 c1, co_eq c0 c1 -> (forall s, s=~ c0 <-> s =~ c1).
Proof.
intros. punfold H. 
 Admitted.
...
Check c_eq_soundness.


Hint Unfold co_eq : core.


Inductive c_eq (co_eq : Contract -> Contract -> Prop) : Contract -> Contract -> Prop :=
| c_plus_assoc c0 c1 c2 : c_eq co_eq ((c0 _+_ c1) _+_ c2) 
                                     (c0 _+_ (c1 _+_ c2))
| c_plus_comm c0 c1: c_eq co_eq (c0 _+_ c1) 
                                (c1 _+_ c0)
| c_plus_neut c: c_eq co_eq (c _+_ Failure) 
                             c
| c_plus_idemp c : c_eq co_eq (c _+_ c)
                              c 
| c_seq_assoc c0 c1 c2 : c_eq co_eq ((c0 _;_ c1) _;_ c2) 
                                    (c0 _;_ (c1 _;_ c2))
| c_seq_neut_l c : c_eq co_eq (Success _;_ c)
                               c 
| c_seq_neut_r c : c_eq co_eq (c _;_ Success)
                               c 
| c_seq_failure_l c : c_eq co_eq (Failure _;_ c) 
                                  Failure  
| c_seq_failure_r c : c_eq co_eq  (c _;_ Failure)
                                  Failure 
| c_distr_l c0 c1 c2 : c_eq co_eq (c0 _;_ (c1 _+_ c2))
                                  ((c0 _;_ c1) _+_ (c0 _;_ c2))
| c_distr_r c0 c1 c2 : c_eq co_eq ((c0 _+_ c1) _;_ c2)
                                 ((c0 _;_ c2) _+_ (c1 _;_ c2))

| c_par_assoc c0 c1 c2 : c_eq co_eq ((c0 _*_ c1) _*_ c2)
                                    (c0 _*_ (c1 _*_ c2))
| c_par_neut c : c_eq co_eq (c _*_ Success)
                             c 
| c_par_comm c0 c1: c_eq co_eq (c0 _*_ c1)
                               (c1 _*_ c0)
| c_par_failure c : c_eq co_eq (c _*_ Failure)
                               Failure   
| c_par_distr_l c0 c1 c2 : c_eq co_eq (c0 _*_ (c1 _+_ c2))
                                      ((c0 _*_ c1) _+_ (c0 _*_ c2))

| c_par_event e0 e1 c0 c1 : c_eq co_eq (Event e0 _;_ c0 _*_ Event e1 _;_ c1) 
                                       (Event e0 _;_ (c0 _*_ (Event e1 _;_ c1)) _+_ Event e1 _;_ ((Event e0 _;_ c0) _*_ c1))

| c_unfold c :  c_eq co_eq (Success _+_ (c _;_ Star c))
                           (Star c)
| c_refl c : c_eq co_eq c c
| c_sym c0 c1 (H: c_eq co_eq c0 c1) : c_eq co_eq c1 c0
| c_trans c0 c1 c2 (H1 : c_eq co_eq c0 c1) (H2 : c_eq co_eq c1 c2) : c_eq co_eq c0 c2
| c_plus_ctx c0 c0' c1 c1' (H1 : c_eq co_eq c0 c0') (H2 : c_eq co_eq c1 c1') : c_eq co_eq (c0 _+_ c1)
                                                                                          (c0' _+_ c1')
| c_seq_ctx c0 c0' c1 c1' (H1 : c_eq co_eq c0 c0') (H2 : c_eq co_eq c1 c1') : c_eq co_eq (c0 _;_ c1)
                                                                                         (c0' _;_ c1')
| c_par_ctx c0 c0' c1 c1' (H1 : c_eq co_eq c0 c0') (H2 : c_eq co_eq c1 c1') : c_eq co_eq (c0 _*_ c1) (c0' _*_ c1')
| c_co_sum  l0 l1 (H0 : length l0 = length alphabet)
                  (H1 : length l1 = length alphabet) 
                  (H2: forall n, co_eq (nth n l0 Failure) (nth n l1 Failure) : Prop) (*Apply coinductive premise here*)
                  : c_eq co_eq (Σe alphabet l0) (Σe alphabet l1).


(*| c_star_ctx c0 c0' (H1 : c0 =R= c0') :  (Star c0 =R= Star c0')*)

Hint Constructors c_eq : eqDB.



(*CoInductive co_eq : Contract -> Contract -> Prop :=
  | _co_eq c0 c1 : c_eq co_eq c0 c1 -> co_eq c0 c1.*)

Definition co_eq c0 c1 := paco2 c_eq bot2 c0 c1.
Hint Unfold co_eq : core.

Notation "c0 =R= c1" := (c_eq co_eq c0 c1)(at level 63).
Notation "c0 =C= c1" := (co_eq c0 c1)(at level 63).


Lemma co_eq_soundness : forall c0 c1, c0 =R= c1 -> Bisimilarity c0 c1.
Proof.
intros. induction H.
25: { pcofix CIH. generalize dependent l1. generalize dependent l0. 
      pcofix CIH. intros. pfold. constructor.
- intros. right.
pcofix CIH.

Lemma co_eq_soundness : forall c0 c1, c0 =C= c1 -> Bisimilarity c0 c1.
Proof.
pcofix CIH.
intros. punfold H0. Check c_seq_failure_l. pclearbot. induction H0.
- fold Bisimilarity. pose proof matches_eq_iff_bisimilarity c0 c1.
  unfold Bisimilarity in H. rewrite <- H.

CoInductive c_eq : Contract -> Contract -> Prop :=
  | _c_eq c0 c1 : c_eq_gen c_eq c0 c1 -> c_eq c0 c1.
...
Proof. auto using c_eq_base_soundness  with eqDB. Qed.

Lemma Matches_plus_neut_l : forall c s, s =~ Failure _+_ c <-> s =~ c.
Proof. intros. rewrite Matches_plus_comm. auto using c_eq_base_soundness  with eqDB. Qed.
...
(*maybe remove alphabet and generalize later*)
Reserved Notation "c0 =C= c1" (at level 63).
CoInductive c_eq : Contract -> Contract -> Prop :=
| co_base c0 c1 (H: c0 =R= c1): c0 =C= c1 
| co_sum l0 l1 (H0 : length l0 = length alphabet) 
               (H1 : length l1 = length alphabet) 
                  (H1: forall n, nth n l0 Failure =C= nth n l1 Failure) : 
         Σe alphabet l0 =C= Σe alphabet l1
  where "c1 =C= c2" := (c_eq c1 c2).

Lemma c_eq_sym : forall c0 c1, c0 =C= c1 -> c1 =C= c0.
Proof.
...
Lemma c_eq_refl : forall c, c =C= c. 
Proof.
cofix H. intros. constructor. reflexivity.
Qed.
...
- rewrite <- matches_eq_is_bisimilarity. auto using c_eq_base_soundness.
- constructor.
  * intros. apply H. apply test. auto. 
  * now repeat rewrite Σe_not_nu.
Qed.

Lemma c_eq_sound : forall c0 c1, c0 =C= c1 -> (forall s, s=~c0 <-> s=~c1).
Proof.
intros c0 c1 H. rewrite matches_eq_is_bisimilarity.
auto using c_eq_sound_aux.
Qed.
...
- apply IHc_eq_base. rewrite Heqc1. rewrite Heqc0.
Lemma not_derive_star : forall (c: Contract),  Star c =R= Success _+_ Σe\ c _;_ Star c -> False. 
Proof.
...
- eauto using test2.
- eapply test3. IHc_eq_base. rewrite Heqc0. apply test3.
